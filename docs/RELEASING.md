...
5. Create an immutable stable tag such as `v0.4.1`.
...
8. Maven Central publication reuses the native resources from that immutable
   GitHub release and publishes the API, Kotlin backend, and LGPL runtime in
   one Gradle invocation and one Central deployment. Every artifact is signed,
   and every POM declares its own license. Public downloadability of the API
   and backend does not change their internal-use terms.
