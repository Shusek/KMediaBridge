...
def verify_central_supporting_archives(pom: Pom, version: str) -> None:
    base_name = f"{pom.artifact_id}-{version}"
    for classifier in ("sources", "javadoc"):
        archive = pom.path.parent / f"{base_name}-{classifier}.jar"
        if not zipfile.is_zipfile(archive):
            raise ValueError(
                f"{pom.path.name} is missing its Maven Central "
                f"{classifier} JAR"
            )


def verify_internal_core_repository(
    repository: Path,
    version: str,
) -> tuple[list[Pom], int]:
    ...
    if None in families or families != {"api", "ffmpeg"}:
        raise ValueError(
            "internal core staging must contain exactly the api and ffmpeg artifact families; "
    ...
    for pom in poms:
        ...
        for archive in archives_for_pom(pom, version):
            ...
            with zipfile.ZipFile(archive) as zipped:
                ...
                if native_names:
                    raise ValueError(
                        f"internal core archive {archive.name} embeds native payloads"
                    )
    ...
    if licensed_families != {"api", "ffmpeg"}:
        raise ValueError(
            "api and ffmpeg publications must each embed the internal-use license"
        )
...
def main() -> int:
    ...
    parser.add_argument("--internal-core-repository", type=Path, required=True)
    parser.add_argument("--runtime-repository", type=Path, required=True)
    parser.add_argument("--version", required=True)
    arguments = parser.parse_args()
    version = arguments.version
    try:
        internal_poms, internal_archives = verify_internal_core_repository(
            arguments.internal_core_repository.resolve(),
            version,
        )
        runtime_poms, native_archives = verify_runtime_repository(
            arguments.runtime_repository.resolve(),
            version,
        )
    except (
    ...
    print(
        f"Verified {len(internal_poms)} internal-use core POMs/"
        f"{internal_archives} archives, "
        f"{len(runtime_poms)} LGPL runtime POM, and "
