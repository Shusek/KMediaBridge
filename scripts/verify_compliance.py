...
def verify_publication_routes(root: Path) -> None:
    for module in ("api", "ffmpeg"):
        ...
        if forbidden:
            fail(
                f"{module} contains an obsolete or runtime-only publication route: "
                + ", ".join(forbidden)
            )
        for required in (
    ...
    for forbidden in (
        ...
        "PrivateCoreRepository",
        "KMEDIA_BRIDGE_PRIVATE_MAVEN",
    ):
    ...
    for obsolete in (
        ":api:publishToMavenCentral",
        ":ffmpeg:publishToMavenCentral",
        ":ffmpeg-runtime-desktop:publishAndReleaseToMavenCentral",
    ):
        if obsolete in central_workflow:
            fail(
                "Maven Central publication must use the aggregate "
                f"publishAndReleaseToMavenCentral task, not {obsolete}."
            )
    if "KMEDIA_BRIDGE_PRIVATE_MAVEN" in central_workflow:
        fail("Maven Central publication must not depend on a private Maven route.")
